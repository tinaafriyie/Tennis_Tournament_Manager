...
Player Management: 128 men's and women's singles players with rankings and statistics
Tournament Progression: 7 rounds from first round to finals
Statistics Tracking: Match/set/game/point statistics, aces, double faults, break points
Referee System: Experienced referees with dispute resolution
Spectator Simulation: Dynamic crowd reactions based on match intensity
Console Interface: User-friendly menu system for tournament management

Class Structure
Core Classes
Playable Interface
Purpose: Defines contract for all playable tennis components
Methods:
    play(): Executes the component until completion
    isComplete(): Checks if component has finished
    getWinner(): Returns the winning player
    Implemented by: Game, Set, Match
    Benefits:
    Enables polymorphism - treat Game/Set/Match uniformly
    Consistent API across all playable components
    Easy to extend with new playable types
    Demonstrates interface-based design pattern
Person (Base Class)
...
Interface Design Pattern: Playable interface for Game, Set, Match
...
Polymorphism: Playable interface enables uniform treatment of Game/Set/Match
...
├── Spectator.java
├── Rally.java
├── Game.java (implements Playable)
├── Set.java (implements Playable)
├── Match.java (implements Playable)
├── Tournament.java
